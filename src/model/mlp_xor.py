...
class MultiLayerPerceptronXOR(Classifier):
    """
    A XOR evaluator based on logistic regression algorithm
    ...
    def __init__(self, train, valid, test, hiddenLayers, outputDim, learningRate=0.01, epochs=30):
        ...
        for layerSize in hiddenLayers:
            self.layers.append(Layer(nIn = previousLayerSize, nOut = layerSize, activation='tanh'))
            previousLayerSize = layerSize
        # Create output layer
        self.layers.append(Layer(nIn = previousLayerSize, nOut = outputDim, activation='tanh'))
    ...
    def train(self):
        ...
        while not learned:
            ...
            for input, label in zip(self.trainingSet.input, self.trainingSet.label):
                self.trainSingleInput(input, [label,])
            ...
            if (prevError - error) < self.errorDeltaThreshold or iteration >= self.epochs:
                learned = True
            prevError = error # Update the error value for the next iteration
            
            logging.info("Iteration: %i; Error: %.10f", iteration, error)
    ...
    def classify(self, testInstance):
        ...
        -------
        float
            1.0 if the MLP evaluates to True, or -1.0 if False
        """
        result = 1.0 if self.fire(testInstance)[0] > 0 else -1.0
        return result
    ...
    def trainSingleInput(self, input, target):
        ...
        for l in xrange(len(layerOutputs)-1, -1, -1):
            ...
            if downstreamSigmas is None: # i.e. this is the output layer
                ...
                sigmas = map(lambda t, o, d: (t - o) * d, target, layerOutputs[l], layerOutputDerivatives[l])
